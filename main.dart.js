...
["ascii_illustrator", "main.dart", , F, {
  "^": "",
  main: [function() {
    F.generateGrid();
    J.remove$0$ax(document.querySelector("span#loading"));
  }, "call$0", "main$closure", 0, 0, 1],
  generateGrid: function() {
    var colors, mainGrid, mainGridBody, t1, i, mainGridRow, t2, j, mainGridCell, mainGridCellContainer, t3, mainGridCellText, subGrid, subGridBody, t4, k, subGridRow, t5, l, subGridCell, t6;
    colors = ["red", "blue", "yellow", "aqua", "green", "black", "grey", "orange", "brown", "coral", "crimson"];
    ...
    for (t1 = J.getInterceptor$x(mainGridBody), i = 1; i <= $.mainDimension; ++i) {
      ...
      for (t2 = J.getInterceptor$x(mainGridRow), j = 1; j <= $.mainDimension; ++j) {
        ...
        mainGridCellContainer.toString;
        t3 = C.C__JSRandom.nextInt$1(11);
        if (t3 < 0 || t3 >= 11)
          return H.ioore(colors, t3);
        mainGridCellContainer.setAttribute("color", colors[t3]);
        mainGridCellText = document.createElement("span", null);
        mainGridCellText.className = "mainGridCellText";
        mainGridCell.appendChild(mainGridCellContainer);
        t3 = J.getInterceptor$x(mainGridCellContainer);
        t3.get$children(mainGridCellContainer).add$1(0, mainGridCellText);
        subGrid = document.createElement("table", null);
        subGrid.className = "subGrid";
        subGridBody = J.createTBody$0$x(subGrid);
        for (t4 = J.getInterceptor$x(subGridBody), k = 1; k <= 5; ++k) {
          subGridRow = t4.insertRow$1(subGridBody, 0);
          for (t5 = J.getInterceptor$x(subGridRow), l = 1; l <= 5; ++l) {
            subGridCell = t5.insertCell$1(subGridRow, 0);
            subGridCell.children;
            t6 = document.createElement("div", null);
            t6.className = "pixel";
            subGridCell.appendChild(t6);
          }
        }
  ...
  PixelMap: {
    ...
    check$1: function(map) {
      var t1, variation, t2, matches, i;
      for (t1 = this._variations, t1 = new J.ArrayIterator(t1, t1.length, 0, null); t1.moveNext$0();) {
        variation = t1.__interceptors$_current;
        t2 = J.getInterceptor$asx(variation);
        if (map.length !== t2.get$length(variation))
          return false;
        for (matches = true, i = 0; i < map.length; ++i)
          if (map[i] !== t2.$index(variation, i))
            matches = false;
        if (matches)
          return true;
      }
    ...
    combine$2: function(map, $char) {
      var resultVariations, t1, variation1, t2, t3, variation2, t4, t5, result, i;
      resultVariations = H.setRuntimeTypeInfo([], [[P.List, P.bool]]);
      for (t1 = this._variations, t1 = new J.ArrayIterator(t1, t1.length, 0, null); t1.moveNext$0();) {
        variation1 = t1.__interceptors$_current;
        for (t2 = map._variations, t2 = new J.ArrayIterator(t2, t2.length, 0, null), t3 = J.getInterceptor$asx(variation1); t2.moveNext$0();) {
          variation2 = t2.__interceptors$_current;
          t4 = J.getInterceptor$asx(variation2);
          if (!J.$eq(t3.get$length(variation1), t4.get$length(variation2)))
            return;
          t5 = t3.get$length(variation1);
          if (typeof t5 !== "number")
            return H.iae(t5);
          result = Array(t5);
          result.fixed$length = Array;
          result.$builtinTypeInfo = [P.bool];
          for (t5 = result.length, i = 0; i < t5; ++i)
            result[i] = t3.$index(variation1, i) === true || t4.$index(variation2, i) === true;
          resultVariations.push(result);
        }
      }
      return new F.PixelMap(resultVariations, $char);
    },
    flip$1: function($char) {
      var resultVariations, t1, variation, result, rows, t2, i, t3;
      resultVariations = H.setRuntimeTypeInfo([], [[P.List, P.bool]]);
      for (t1 = this._variations, t1 = new J.ArrayIterator(t1, t1.length, 0, null); t1.moveNext$0();) {
        variation = t1.__interceptors$_current;
        result = [];
        result.$builtinTypeInfo = [P.bool];
        rows = [];
        rows.$builtinTypeInfo = [[P.List, P.bool]];
        t2 = J.getInterceptor$asx(variation);
        i = 0;
        while (true) {
          t3 = t2.get$length(variation);
          if (typeof t3 !== "number")
            return H.iae(t3);
          if (!(i < t3))
            break;
          rows.push(t2.sublist$2(variation, i, i + $.dimension));
          i += $.dimension;
        }
        for (i = rows.length - 1; i >= 0; --i) {
          if (i >= rows.length)
            return H.ioore(rows, i);
          C.JSArray_methods.addAll$1(result, rows[i]);
        }
        resultVariations.push(result);
      }
      return new F.PixelMap(resultVariations, $char);
    ...
    toString$0: function(_) {
      var t1, output, variation, t2, y, t3, x;
      for (t1 = this._variations, t1 = new J.ArrayIterator(t1, t1.length, 0, null), output = "["; t1.moveNext$0();) {
        variation = t1.__interceptors$_current;
        for (t2 = J.getInterceptor$asx(variation), y = 0; y < $.dimension; ++y) {
          for (t3 = y * 5, x = 0; x < $.dimension; ++x)
            output += (t2.$index(variation, x + t3) === true ? "*" : " ") + " ";
          output += "\n";
        }
        output += "-\n";
      }
    ...
    PixelMap$2: function(map, $char) {
      this._variations.push(map);
    },
    static: {PixelMap$: function(map, $char) {
        var t1 = new F.PixelMap(H.setRuntimeTypeInfo([], [[P.List, P.bool]]), $char);
        t1.PixelMap$2(map, $char);
        return t1;
      }, PixelMap$variations: function(maps, $char) {
  ...
  generateGrid_closure: {
    "^": "Closure:5;",
    call$1: function(me) {
      var target, t1, t2, list;
      target = H.interceptedTypeCast(J.get$target$x(me), "$isDivElement");
      t1 = target.style;
      t2 = target.parentElement.parentElement.parentElement.parentElement.parentElement.getAttribute("color");
      t1.toString;
...
["dart.async", "dart:async", , P, {
  ...
  }, "call$1", "_nullDataHandler$closure", 2, 0, 20],
  ...
  }, null, "call$2", "call$1", "_nullErrorHandler$closure", 2, 2, 8, 0],
  ...
  _AsyncRun__initializeScheduleImmediate_closure: {
    "^": "Closure:6;_box_0,_captured_div_1,_captured_span_2",
  ...
  _BroadcastSubscription: {
    "^": "_ControllerSubscription;_eventState,_next@,_previous?,_async$_controller,_onData,_onError,_onDone,_zone,_state,_cancelFuture,_pending",
  ...
  _Future__propagateToListeners_handleWhenCompleteCallback_closure0: {
    "^": "Closure:9;_box_0,_captured_result_12",
    call$2: function(error, stackTrace) {
...
["dart.collection", "dart:collection", , P, {
  ...
  }, "call$1", "_defaultHashCode$closure", 2, 0, 22],
...
}, "undefinedLiteralPropertyPattern", "slash", "get$slash", function() {
  return F.PixelMap$variations([[false, false, false, false, true, false, false, false, true, false, false, false, true, false, false, false, true, false, false, false, true, false, false, false, false], [false, false, false, true, true, false, false, true, true, false, false, true, true, false, false, true, true, false, false, false, true, false, false, false, false], [false, false, false, false, true, false, false, false, true, true, false, false, true, true, false, false, true, true, false, false, true, true, false, false, false]], "/");
}, "slash", "pipe", "get$pipe", function() {
  return F.PixelMap$([false, false, true, false, false, false, false, true, false, false, false, false, true, false, false, false, false, true, false, false, false, false, true, false, false], "|");
}, "pipe", "dash", "get$dash", function() {
  return F.PixelMap$([false, false, false, false, false, false, false, false, false, false, true, true, true, true, true, false, false, false, false, false, false, false, false, false, false], "\u2013");
}, "dash", "topBar", "get$topBar", function() {
  return F.PixelMap$([true, true, true, true, true, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false], "\u00af");
}, "topBar", "underscore", "get$underscore", function() {
  return F.PixelMap$([false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, true, true, true, true, true], "_");
}, "underscore", "l", "get$l", function() {
  return F.PixelMap$([true, false, false, false, false, true, false, false, false, false, true, false, false, false, false, true, false, false, false, false, true, true, true, true, true], "L");
}, "l", "equals", "get$equals", function() {
...
}, "plusMinus", "notEquals", "get$notEquals", function() {
  return $.get$equals().combine$2($.get$slash(), "\u2260");
}, "notEquals", "backSlash", "get$backSlash", function() {
  return $.get$slash().flip$1("\\");
}, "backSlash", "x", "get$x", function() {
  return $.get$slash().combine$2($.get$backSlash(), "X");
}, "x", "T", "get$T", function() {
